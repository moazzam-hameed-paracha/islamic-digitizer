...
const PREDICT_URL = `${GRADIO_BASE_URL}/api/predict`;
...
function deriveMetadata(text: string) {
  ...
  const hasTables = nonEmpty.some(
    (l) => (l.match(/\|/g) ?? []).length >= 2 || (l.match(/\t/g) ?? []).length >= 2
  );
...
export async function POST(req: NextRequest) {
  try {
    ...
    const gradioRes = await fetch(PREDICT_URL, {
      method: "POST",
    ...
    if (!gradioRes.ok) {
      ...
      return NextResponse.json(
        ...
        { status: gradioRes.status }
      );
    }

    const gradioData = await gradioRes.json();

    // Gradio wraps all outputs in a "data" array; index 0 is our Textbox value.
    const rawText: string =
      typeof gradioData?.data?.[0] === "string" ? gradioData.data[0] : "";

    const { confidence, metadata } = deriveMetadata(rawText);

    return NextResponse.json({ text: rawText, confidence, metadata });
  } catch (error) {
